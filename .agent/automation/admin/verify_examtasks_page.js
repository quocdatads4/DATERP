...
/**
 * verify_examtasks_page.js
 * Kiểm tra trang Nhiệm vụ (ExamTasks)
 * - Đăng nhập Admin
 * - Truy cập trang ExamTasks
 * - Kiểm tra bảng hiển thị đúng
 */

const LOG_DIR = path.join(__dirname, '..', '..', 'log');
const LOG_FILE = path.join(LOG_DIR, `examtasks_automation_${new Date().toISOString().replace(/[:.]/g, '-')}.log`);
...
(async function verifyExamTasksPage() {
    ...
    try {
        log('=== EXAM TASKS PAGE VERIFICATION ===');
        ...
        // 2. Truy cập trực tiếp trang ExamTasks
        log('Step 2: Navigating to ExamTasks page...');
        await driver.get('http://localhost:5223/Examination/ExamTasks');
        await driver.sleep(3000);

        let currentUrl = await driver.getCurrentUrl();
        if (currentUrl.includes('/Examination/ExamTasks')) {
            log('SUCCESS: Navigated to ExamTasks page.');
        } else {
        ...
        try {
            let table = await driver.wait(
                until.elementLocated(By.id('ExamTasksTable')),
                10000
            );
            log('SUCCESS: ExamTasksTable found.');

            // Kiểm tra các cột
            let headers = await driver.findElements(By.css('#ExamTasksTable thead th'));
            ...
            log('WARNING: ExamTasksTable not found or not loaded yet.');
        }

        // 5. Kiểm tra nút tạo mới
        log('Step 5: Checking Create button...');
        try {
            let createBtn = await driver.findElement(By.id('NewExamTaskButton'));
            let btnText = await createBtn.getText();
            log(`SUCCESS: Create button found with text: "${btnText}"`);
        } catch (e) {
            log('WARNING: New ExamTask button not found.');
        }
        ...
        log('');
        log('✅ EXAM TASKS PAGE VERIFICATION COMPLETED.');
        ...
        try {
            ...
            let screenshotPath = path.join(LOG_DIR, 'examtasks_error_screenshot.png');
