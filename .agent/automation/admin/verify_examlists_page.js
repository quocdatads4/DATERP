...
/**
 * verify_examlists_page.js
 * Kiểm tra trang Danh sách bài thi (ExamLists)
 * - Đăng nhập Admin
 * - Truy cập trang ExamLists
 * - Kiểm tra bảng hiển thị đúng
 */

const LOG_DIR = path.join(__dirname, '..', '..', 'log');
const LOG_FILE = path.join(LOG_DIR, `examlists_automation_${new Date().toISOString().replace(/[:.]/g, '-')}.log`);
...
(async function verifyExamListsPage() {
    ...
    try {
        log('=== EXAM LISTS PAGE VERIFICATION ===');
        ...
        // 2. Truy cập trực tiếp trang ExamLists
        log('Step 2: Navigating to ExamLists page...');
        await driver.get('http://localhost:5223/Examination/ExamLists');
        await driver.sleep(3000);

        let currentUrl = await driver.getCurrentUrl();
        if (currentUrl.includes('/Examination/ExamLists')) {
            log('SUCCESS: Navigated to ExamLists page.');
        } else {
        ...
        try {
            let table = await driver.wait(
                until.elementLocated(By.id('ExamListsTable')),
                10000
            );
            log('SUCCESS: ExamListsTable found.');

            // Kiểm tra các cột
            let headers = await driver.findElements(By.css('#ExamListsTable thead th'));
            ...
            log('WARNING: ExamListsTable not found or not loaded yet.');
        }

        // 5. Kiểm tra nút tạo mới
        log('Step 5: Checking Create button...');
        try {
            let createBtn = await driver.findElement(By.id('NewExamListButton'));
            let btnText = await createBtn.getText();
            log(`SUCCESS: Create button found with text: "${btnText}"`);
        } catch (e) {
            log('WARNING: New ExamList button not found.');
        }
        ...
        log('');
        log('✅ EXAM LISTS PAGE VERIFICATION COMPLETED.');
        ...
        try {
            ...
            let screenshotPath = path.join(LOG_DIR, 'examlists_error_screenshot.png');
