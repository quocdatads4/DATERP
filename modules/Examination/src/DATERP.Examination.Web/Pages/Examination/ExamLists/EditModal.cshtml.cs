...
public class EditModalModel : ExaminationPageModel
{
    ...
    [BindProperty]
    public EditExamListViewModel ExamList { get; set; } = default!;
    ...
    {
        var examListDto = await _examListAppService.GetAsync(Id);
        ExamList = new EditExamListViewModel
        {
            SubjectId = examListDto.SubjectId,
            Title = examListDto.Title,
            TimeLimit = examListDto.TimeLimit,
            Order = examListDto.Order
        };
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        await _examListAppService.UpdateAsync(Id, dto);
        return NoContent();
    }
    ...
    private async Task LoadSubjectsAsync()
    {
        ...
        Subjects = subjects.Items.Select(x => new SelectListItem
        {
            Value = x.Id.ToString(),
            Text = x.Name,
            Selected = x.Id == ExamList.SubjectId
        }).ToList();
    }

    public class EditExamListViewModel
    {
